...
private:
  ...
  const QDir m_QwertycoinDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfQwertycoinBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _Qwertycoin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo QwertycoinBlockchainInfo() const;
